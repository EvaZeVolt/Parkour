var Player, controller, loop, Blocks, Blocks2, Blocks3, JumpPad, float;
...
loop = function () {
  ...
  if (Player.y > 202 && Player.y < 220 && Player.x > 400 && Player.x < 640) {
    Player.jumping = false;
    Player.y = 350;
  }
  if (Player.x > 150 && Player.x < 400 && Player.y > 130 && Player.y < 200) {
    Player.y = 150;
    Player.jumping = false;
    Player.y_velocity = 1;
  }
  if (Player.x > 100 && Player.x < 150 && Player.y < float.y) {
  ...
  ctx.fillStyle = "#363836";
  ctx.fillRect(Blocks3.x, Blocks3.y, Blocks3.width, Blocks3.height);
